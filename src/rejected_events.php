...
<head>
    <meta charset="UTF-8">
    <title>Rejected Events | BookLoop Admin</title>
...
<body class="bg-gray-100 p-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-red-700">Rejected Events</h1>
        <div class="space-x-4">
            <a href="admin_dashboard.php" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Pending Events</a>
            <a href="approved_events.php" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">Approved Events</a>
        </div>
    </div>
            ...
            <?php while ($event = $result->fetch_assoc()): ?>
                <div class="bg-white p-4 rounded-lg shadow-lg flex gap-4 items-center">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-red-600 font-semibold">Rejected</p>
                        <?php if (!empty($event['rejection_reason'])): ?>
                            <p class="text-gray-700"><strong>Reason:</strong> <?= htmlspecialchars($event['rejection_reason']) ?></p>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
            <p class="text-gray-500">No rejected events found.</p>
        <?php endif; ?>
    </div>
</body>
</html>